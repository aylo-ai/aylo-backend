# Pricing ladder rework: 699,000 popular tier + custom all-inclusive Pro tier
...
Make the popular package cost **699,000 UZS for 2,000 conversations**, and make
the last (**Pro**) package a custom-priced, custom-volume tier with everything
included. Decisions confirmed before implementing:

| Decision | Choice |
|---|---|
| Ladder shape | Three tiers: Free → Basic (699k) → Pro (custom) |
| What the custom tier does | Not purchasable self-service; it collects a contact-sales request |
| Naming | Free / Basic / Pro — the repo's original naming, with Pro as the top all-inclusive tier |

## The ladder now

| Tier | `type` | Price | Conversations / tokens | Features | Popular | Buyable self-service |
|---|---|---|---|---|---|---|
| Free | `free` | 0 | 100 | starter set | no | yes |
| Basic | `basic` | 699,000 UZS | 2,000 | all but team seats / priority support | **yes** | yes |
| Pro | `custom` | — (negotiated) | agreed per company | **everything** | no | **no — quote request** |

Verified against the running API:

```
Free         0 UZS            conversations=100      popular=False
Basic        699,000 UZS      conversations=2000     popular=True
Pro          custom           conversations=custom   popular=False
```

`PricingPackageType` gains a `BASIC` value so the stored type matches the tier
name. `CUSTOM` describes the *pricing model* — agreed per company — which is why
the tier named "Pro" carries `type=custom`.

`Korporativ` (the name used in the first draft of this branch) is retired by the
seed command. Retirement **deactivates** rather than deletes: existing
subscriptions still point at that row, and `Subscription.pricing_package` is the
only record of what a customer bought.

## Changes

### 1. The custom tier is a first-class concept

`PricingPackageType.CUSTOM` means "priced per company" — it was previously a
stray label on the old `Basic` plan, which was just an ordinary paid tier.
...
`expected_conversations` is how the company names the volume it needs — the
"custom token" half of the tier. Returns 400 if the package is not a custom one,
so the form cannot be used as a generic lead sink on a priced tier.
...
pricing page led with whichever package was seeded last (in practice: the custom
tier first). It now returns cheapest-first with the custom tier pinned last,
via a `Case`/`When` annotation on `type`.
...
`LandingLeadCreateTests` patches the send target, so its patch targets moved
with the code.
...
- `apps/landing/serializers.py` — the phone-number error was the only
  ...
  translated.
- Blog seed price claims ("so'mdan boshlanadi", RU and EN variants) synced to
  699,000 — the seed command's own comment asks for these to be kept in step.
...
| File | Change |
|---|---|
| `apps/shared/addons/enums.py` | `PricingPackageType.BASIC` added |
...
| `apps/payment/migrations/0024_alter_pricingpackage_type.py` | New (`BASIC` added to the type choices) |
...
| `apps/payment/management/commands/seed_pricing_packages.py` | Free / Basic (699k, 2000) / Pro (custom, all features); retires `Korporativ` |
| `apps/payment/tests.py` | Conflict resolved; new `CustomPackageTests`; fixtures retyped; payload contract test updated |
...
| `apps/blog/management/commands/seed_blog_posts.py` | Price claims → 699,000 (uz/ru/en) |
| `locale/*/LC_MESSAGES/django.po` | 5 new msgids × 5 languages |
| `docs/testing/ENDPOINT_*.md` | Regenerated for the new route |

## Tests

`CustomPackageTests` (11 cases):

| Test | Proves |
|---|---|
| `test_the_list_marks_the_custom_tier` | `is_custom` reaches the pricing page |
| `test_the_list_comes_back_as_a_ladder` | Free → Basic → Pro order |
| `test_the_popular_tier_carries_its_price_and_conversation_limit` | 699,000 / 2,000 / popular |
...
```
$ manage.py test apps.payment.tests.CustomPackageTests apps.payment.tests.PlanSelectionTests \
      apps.payment.tests.PublicCatalogueTests \
      apps.payment.tests.PricingPackageValidationTests apps.landing --keepdb
Ran 50 tests in 0.415s
OK
```
...
```
$ manage.py test apps.payment apps.landing apps.shared.tests.test_i18n_catalogs --keepdb
Ran 92 tests in 1.063s
...
The seed command was also run end to end against a real database:

```
Updated Free (0 uzs, 100 requests)
Created Basic (699000 uzs, 2000 requests)
Updated Pro (negotiated, 0 requests)
Deactivated 1 retired package(s): Korporativ
```

## Open items for a human

1. **The old `Pro` package is overwritten, not replaced.** The seed matches on
   name, so the pre-existing `Pro` (699,000 / 10,000 conversations) becomes the
   custom tier when the command runs. Subscriptions pointing at it keep their
   own `remained_request_count`, but the package they reference now reads as
   quote-only. Check this before running the seed in production.
2. **`origin/dev` has two more broken files.** `apps/integration/tests.py` is
   ...
   source-string tests cannot run — they parse every file in `apps/`.
3. **Five payment tests already fail on `origin/dev`** (`CardTokenBindingTests`,
   `PaymeVerificationThrottleTests`) — unrelated to pricing, but worth a look
   before the next release.
4. **Conversation limit for a custom subscription.** `Pro` is seeded with
   `request_count = 0`. When sales closes a deal, an admin has to set the agreed
   conversation/token count on the subscription (or create a per-company
...
5. **Blog price claims.** Updated in the seed command, but any post already
   written to the production database still carries the old figure — re-run
