...
from .sql_server import SqlDriver
...
class SafeSqlDriver(SqlDriver):
    """
    A safe SQL driver that uses pglast to parse and execute queries.
    """
    ...
    async def execute(self,
        ...
        if self.timeout:
            try:
                ...
                logging.error(f"Query execution timed out after {self.timeout} seconds: {query}")
    ...
    def sql_to_query(sql: Composable) -> str:
        """Convert a Composable object to a SQL string"""
    ...
    @staticmethod
    async def execute_param_query(sql_driver:SqlDriver, query: LiteralString,
                                  params: list[Any]) -> List[SqlDriver.RowResult]:
